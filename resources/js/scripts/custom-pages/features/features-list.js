...
$(function () {
    ...
    if (dtUserTable.length) {
        dtUserTable.DataTable({
            ajax: {
                url: '/admin/allFeatures', // JSON file to add data
                dataSrc: 'content'
            },
            columns: [
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        var $name = full['title'], $image = full['image'];
                ...
                },
                {
                    ...
                    targets: 2,
                    render: function (data, type, full, meta) {
                        var $reason = full['desc'];

                        var $out = '';

                        $out = '<a href="javascript:;" class="dropdown-item"  data-bs-toggle="modal" ' +
                            'data-bs-target="#info-sub" data-reason="' + $reason + '" data-id="' +
                            full['id']
                            + '" data-title="'+ full['title'] +'">' +
                            $reason.slice(0, 20) +
                            '...' +
                            '<span style="color: blue">' +
                            '' + Lang.get('js_local.Read') +
                            '</span>' +
                            '</a>'
...

                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        out = '<div class="btn-group">' +
                            ...
                            '<a   class="dropdown-item" href="/admin/edit-feature/' +
                            full['id']
                            + '">' +
    ...
    $('#info-sub').on('show.bs.modal', function (e) {
        ...
        let desc = $(e.relatedTarget).data('reason');

        $('#trip')[0].innerHTML =  title;
        $('#reason')[0].innerHTML = desc;
    ...
    $("#delete_btn").on('click', function (e) {
        ...
        $.ajax({
            ...
            url: '/admin/delete-feature/' + sub_id,
            ...
            success: function (response) {
                if (response.status === 200) {
                    ...
                    setTimeout(function () {
                        toastr['success'](
                            Lang.get('js_local.Feature_Deleted_Successfully'),
                            {
                                closeButton: true,
                                tapToDismiss: false,
                                rtl: isRtl
